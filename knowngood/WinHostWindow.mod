MODULE WinHostWindow;

IMPORT SYSTEM, H := WinHost, K := Kernel, Fonts := WinFonts;
...
PROCEDURE LastError;
BEGIN H.ws("Last error: $"); H.wh(H.GetLastError()); H.wsn(".");
END LastError;
...
BEGIN
  H.ws("Fill rectangle. x "); H.wi(x);
  H.ws(", y ");               H.wi(y);
  H.ws(", w ");               H.wi(wi);
  H.ws(", h ");               H.wi(h);
  H.ws(", colour $");         H.wh(colour);
  H.wsn(".");
...
PROCEDURE Quit*;
BEGIN H.PostQuitMessage(0) END Quit;

PROCEDURE Move*(window: Window; x, y: INTEGER);
BEGIN
  H.MoveWindow(window.hwnd, x, y, window.width, window.height, 0);
  window.x := x;  window.y := y
END Move;

PROCEDURE Resize*(window: Window; w, h: INTEGER);
BEGIN
  H.MoveWindow(window.hwnd, window.x, window.y, w, h, 1);
...
BEGIN
  (*
  H.ws("RenderAlphaMapToBitmap, x "); H.wi(x);
  H.ws(", width ");           H.wi(width);
  H.ws(", height ");          H.wi(height);
  H.ws(", mapadr $");         H.wh(mapadr);  H.wsn(".");
...
BEGIN
  ...
  H.ws("Icon dimensions w "); H.wi((glyph.mapWidth+3) DIV 4);
  H.ws(", h "); H.wi(glyph.mapHeight); H.wsn(".");
  ...
  IconInfo.xHotspot := 0;
  IconInfo.yHotspot := 0;
  IconInfo.hbmMask  := H.CreateBitmap(32, 32, 1,  1, SYSTEM.ADR(maskbits));
  IconInfo.hbmColor := H.CreateBitmap(32, 32, 1, 32, SYSTEM.ADR(colourbits));
  icon := H.CreateIconIndirect(SYSTEM.ADR(IconInfo));
  ASSERT(icon # 0);
  ASSERT(H.DeleteObject(IconInfo.hbmMask)  # 0);
  ASSERT(H.DeleteObject(IconInfo.hbmColor) # 0);
...
BEGIN window := FirstWindow;
  (*
  H.ws("Find window $"); H.wh(hwnd);
  IF window = NIL THEN H.wsn(", no windows!")
  ELSE H.ws(", starting with $"); H.wh(window.hwnd); H.wsn(".") END;
  *)
...
BEGIN
  ...
  (*
  H.ws("BitBlt x "); H.wi(x);      H.ws(", y ");      H.wi(y);
  H.ws(", width ");  H.wi(width);  H.ws(", height "); H.wi(height);  H.wsn(".");
  *)
...
BEGIN
  ...
  H.SetHWnd(hwnd);  (* Make sure kernel error message boxes stop the message pump *)

  EnsureBitmap(width, height, window.bmp);

  window.DPI := H.GetDpiForWindow(hwnd);
...
BEGIN quit := FALSE;
  ...
  WHILE ~quit & (res # 0) DO
    ASSERT(res # -1);
    H.ws("PeekMessageW: "); WriteWindowsMessageName(msg.message); H.wn;
    IF msg.message = 0FH THEN H.wsn("** PeekMessage returned WM_PAINT **") END;
...
(*
  ...
  IF ~quit & (GetQueueStatus(20H) # 0) THEN  (* Check for pending WM_PAINT *)
    ...
    H.ws("GetMessageW: "); WriteWindowsMessageName(msg.message); H.wn;
    IF  res # 0 THEN
      res := H.TranslateMessage(SYSTEM.ADR(msg));
      res := H.DispatchMessageW(SYSTEM.ADR(msg))
    END
...
BEGIN H.MsgWaitForMultipleObjects(0, 0, 0, time, 1DFFH);
END WaitMsgOrTime;
...
VAR
  ...
  res:  INTEGER;
BEGIN
  res := H.PeekMessageW(SYSTEM.ADR(msg), 0, 200H, 209H, 1); (* Any mouse move w/wout click *)
  IF res # 0 THEN
...
BEGIN
  ...
  ASSERT(H.InvalidateRect(w.hwnd, SYSTEM.ADR(rect), 0) # 0)
END InvalidateRect;
