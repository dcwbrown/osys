...
(* DCWB 2024/03/22 Adapt from 32 bit to 64 bit INTEGERs and SETs *)
...
BEGIN
  a := Base + (x DIV 64) * 8 + y * 128;
  s := {x MOD 64};
  SYSTEM.GET(a, u);  (* Load 64 bits containing x,y *)
...
BEGIN
  ...
  ELSIF ar > al THEN
    left  := {(x MOD 64) .. 63};
    right := {0 .. ((x+w-1) MOD 64)};
    FOR a0 := al TO al + (h-1)*128 BY 128 DO
      H.ws("ReplConst a0 "); H.wh(a0); H.wc("H");
      SYSTEM.GET(a0, pixl);
      H.ws(", pixl "); H.wh(SYSTEM.VAL(INTEGER, pixl));
      H.ws("H, ar "); H.wh(ar); H.wh(h);
      SYSTEM.GET(ar, pixr);
      H.ws(", pixr "); H.wh(SYSTEM.VAL(INTEGER, pixr)); H.wsn("H.");
      IF mode = invert THEN
        SYSTEM.PUT(a0, pixl / left);
        FOR a1 := a0+8 TO ar-8 BY 8 DO SYSTEM.GET(a1, pix); SYSTEM.PUT(a1, -pix) END;
        SYSTEM.PUT(ar, pixr / right)
      ELSIF (mode = replace) & (col = black) THEN (*erase*)
        SYSTEM.PUT(a0, pixl - left);
        FOR a1 := a0+8 TO ar-8 BY 8 DO SYSTEM.PUT(a1, {}) END;
        SYSTEM.PUT(ar, pixr - right)
      ELSE (* (mode = paint) OR (mode = replace) & (col # black) *)
        SYSTEM.PUT(a0, pixl + left);
        FOR a1 := a0+8 TO ar-8 BY 8 DO SYSTEM.PUT(a1, {0 .. 63}) END;
        SYSTEM.PUT(ar, pixr + right)
      END;
      INC(ar, 128)
    END
...
BEGIN SYSTEM.GET(patadr, w); SYSTEM.GET(patadr+1, h); INC(patadr, 2);
  ...
  FOR a0 := a TO a + (h-1)*128 BY 128 DO
    ...
    END;
    SYSTEM.GET(a0, pix);
    IF mode = invert THEN SYSTEM.PUT(a0, SYSTEM.VAL(SET, LSL(pwd, x MOD 64)) / pix)
    ELSE SYSTEM.PUT(a0, SYSTEM.VAL(SET, LSL(pwd, x MOD 64)) + pix)
    END;
    IF (x MOD 64) + w > 32 THEN (*spill over*)
      SYSTEM.GET(a0+4, pix);
      IF mode = invert THEN SYSTEM.PUT(a0+4, SYSTEM.VAL(SET, ASR(pwd, -(x MOD 64))) / pix)
      ELSE SYSTEM.PUT(a0+4, SYSTEM.VAL(SET, ASR(pwd, -(x MOD 64))) + pix)
      END
    END
  END
END CopyPattern;
...
VAR sa, da, sa0, sa1, d, len: INTEGER;
    ...
    m0, m1, m2, m3: SET;
BEGIN
  u0 := sx DIV 32; u1 := sx MOD 64; u2 := (sx+w) DIV 32; u3 := (sx+w) MOD 64;
  v0 := dx DIV 32; v1 := dx MOD 64; v2 := (dx+w) DIV 32; v3 := (dx+w) MOD 64;
