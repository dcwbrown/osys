MODULE Display;  (*NW 5.11.2013 / 3.7.2016*)
  ...
  BEGIN
    SYSTEM.GET(a, dw);  ls := SYSTEM.VAL(SET, ORD(dw));
  ...
  BEGIN SYSTEM.GET(patadr, w); SYSTEM.GET(patadr+1, h); INC(patadr, 2);
    ...
    FOR a0 := a TO a + (h-1)*Stride BY Stride DO
      ...
      END ;
      Get(a0, pix);
      IF mode = invert THEN Put(a0, SYSTEM.VAL(SET, LSL(pwd, x MOD 32)) / pix)
      ELSE Put(a0, SYSTEM.VAL(SET, LSL(pwd, x MOD 32)) + pix)
      END ;
      IF (x MOD 32) + w > 32 THEN (*spill over*)
        Get(a0+4, pix);
        IF mode = invert THEN Put(a0+4, SYSTEM.VAL(SET, ASR(pwd, 32-(x MOD 32))) / pix)
        ELSE Put(a0+4, SYSTEM.VAL(SET, ASR(pwd, 32-(x MOD 32))) + pix)
        END
      END
    END;
    WinGui.Dirty(x, y, w, h)
